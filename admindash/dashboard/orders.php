                  ...
                  <table class="table">
                    ...
                    <tbody>
                      <tr>
                        <?php
                        $i = 0;
        if($stmt = $conn->query("SELECT * FROM orders")){
          while ($row = $stmt->fetch_assoc()) {
           ...
           <tr style="background-color:white; font-size: 14px;">
                ...
                if($row['paid_status']==1){
                  echo "<a class='btn btn-sm btn-success' href='?type=status&operation=deactive&id=".$row['id']."'>Paid</a>&nbsp;";
                }else{
                  echo "<a class='btn btn-sm btn-pending' href='?type=status&operation=active&id=".$row['id']."'>Unpaid</a>&nbsp;";
                }
                echo "<a class='btn btn-sm btn-primary' href='updateorder.php?id=".$row['id']."'>Edit</a>&nbsp;";
